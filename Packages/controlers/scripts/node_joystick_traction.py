...
def node_joystick_traction():
    ...
    pub_traction_orders = rospy.Publisher('topic_traction_orders', Twist, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        if axis_moved:
            ...
            twistMessage  = Twist()    
            twistMessage.linear.x, twistMessage.linear.y = rigth, left
            #twistMessage.header.stamp = rospy.Time.now()
            # not define for twist twistMessage.header.seq = order.header.seq + 1
            #twistMessage.sensibility = int(max_rpm*(-axis3+1)/2)
            pub_traction_orders.publish(twistMessage)
            axis_moved = False
